...
public class HaliGaliServer {
	...
	public void setAllCard() {//카드 번호 생성
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 14; j++) {
				allCard[i][j] = j;
			}
	...
	public HaliGaliServer() {//생성자
		setAllCard();// 카드셋팅
		cardShuffle(allCard, 200);// 카드섞기
		cardDivide();// 4등분하기
		Collections.synchronizedList(playerStream);//동기화
...
//				}
	...
	class TestThread extends Thread {
		...
		public String fruitsValidate() {
			...
			int[] fru0 = new int[4];//바나나
			int[] fru1 = new int[4];//레몬
			int[] fru2 = new int[4];//자두
			int[] fru3 = new int[4];//딸기
			...
			if ((fruitSum0 != 0 && fruitSum0 % 5 == 0) //ex : 0 3 1 5 : 성공
		...
		public String findCardIndex(String card) {//클라이언트 과일이미지 인덱스와 비교하기위해서 
			String idx = null;
...
//						System.out.println("1P카드수 : "+player01.size());
//						System.out.println("2P카드수 : "+player02.size());
//						System.out.println("3P카드수 : "+player03.size());
//						System.out.println("4P카드수 : "+player04.size());
//						
						try {
							if (player01.size() == 0) {
								playerSu.remove("0key");
